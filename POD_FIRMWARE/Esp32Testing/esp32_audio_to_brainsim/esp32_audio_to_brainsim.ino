...
const i2s_config_t i2s_mic_config = {
    ...
    .dma_buf_count = 1,                           // number of buffers
...
void Record_and_Send() {
    ...
    if(speaker_on_flag == true){
      i2s_driver_uninstall(I2S_PORT);      
      speaker_on_flag = false;
    }
    if(mic_on_flag == false){
      err = i2s_driver_install(I2S_PORT, &i2s_mic_config, 0, NULL);
      if (err != ESP_OK) {
    ...
    if (bytes_read > 0) {
        bonus[0] = sample >> 24;
        //Masking the sample byte by byte for transfer via udp
        convVal[0] = (sample & 0xff000000UL) >> 24;
        convVal[1] = (sample & 0x00ff0000UL) >> 16;
        convVal[2] = (sample & 0x0000ff00UL) >> 8;
        convVal[3] = (sample & 0x000000ffUL);


        if (bufferChain == 0) {//This initializes udp and transfer buffer    
            udp.beginPacket(udpAddress, udpPort);
        }
        //add the masked values to the udp buffer
        udp.write(convVal[0]);
        udp.write(convVal[1]);
        udp.write(convVal[2]);
        udp.write(convVal[3]);
        ...
        //udp.write(sample);
        bufferChain++;
        if (bufferChain >= 256)//building up 1kB packet to send via udp
        {
            udp.endPacket();//Send via UDP
            bufferChain = 0;
        }
        //Serial.println(sample);//for visualizing the data coming out
    }
}
...
void checkUDPAndRespond() {    
    if (pollUDPListener() != 0) {//Pull the udp payload in, select the audio amplifier on the i2s interface, and play the payload through it;        
        if(mic_on_flag == true){
          i2s_stop(I2S_PORT);        
        ...
        if(speaker_on_flag == false){
          ...
          i2s_set_pin(I2S_PORT, &i2s_speaker_pins);
