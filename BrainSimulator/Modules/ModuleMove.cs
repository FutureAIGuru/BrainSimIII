...
namespace BrainSimulator.Modules
{
    public class ModuleMove : ModuleBase
    {
        ...
        public override void Fire()
        {
            ...
            Module3DSim m3D = (Module3DSim)FindModule(typeof(Module3DSim));
            if (m3D != null && motionX != 0)
                m3D.Move(motionX);

            bool moved = false;
            Module2DSim m2D = (Module2DSim)FindModule(typeof(Module2DSim));
            if (m2D != null && (motionX != 0 || motionY != 0))
                moved = m2D.Move(motionX, motionY);

            Module2DModel m2DModel = (Module2DModel)FindModule(typeof(Module2DModel));
            if (m2DModel != null && moved && motionX != 0 || motionY != 0)
                m2DModel.Move(motionX, motionY);

            Module2DVision m2DVision = (Module2DVision)FindModule(typeof(Module2DVision));
