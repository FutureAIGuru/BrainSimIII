...
public class ModuleShape : ModuleBase
{
            ...
            if (foundShapes.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (newStoredShape != null)
                    {
                        ...
                        if (MNISTDigit == "")
                            newStoredShape.Relationships.FindFirst(x => x.reltype.Label == "is-a").TimeToLive = TimeSpan.FromSeconds(secondsForTemporaryShapes);
    ...
    private void ExtractShape(Thing currentShape, Thing outline)
    {
        ...
        for (int i = 0; i < corners.Count; i++)
        {
            ...
            //if (theCorner.prevPt == theCorner.nextPt) continue;
