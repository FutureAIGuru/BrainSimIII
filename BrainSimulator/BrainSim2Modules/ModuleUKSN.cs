...
namespace BrainSimulator.Modules
{
    public class ModuleUKSN : ModuleUKS
   {
        ...
        public override void Initialize()
        {
            ...
            UKSList.Insert(0, new Thing { Label = "Sleep" });
            UKSList.Insert(0, new Thing { Label = "Max" });
            UKSList.Insert(0, new Thing { Label = "Ref" });
            UKSList.Insert(0, new Thing { Label = "Parent" });
            firstThing = 4;
            EventCount = 0;
            phraseCount = 0;
        ...
        {
            for (int i = firstThing; i < mv.NeuronCount && i < UKSList.Count; i++)
            {
                Neuron n = mv.GetNeuronAt(i);
                if (NeuronFired("Parent"))
                {
                    if (n.Fired())
                    {
                        foreach (Thing t in UKSList[i].Parents)
                        {
                            int j = UKSList.IndexOf(t);
                            if (j >= 0)
                            {
                ...
                if (NeuronFired("Ref"))
                {
                    if (n.Fired())
                    {
                        foreach (Link l in UKSList[i].References)
                        {
                            int j = UKSList.IndexOf(l.T);
                            if (j >= 0)
                            {
        ...
        private void HandleTalking()
        {
            ...
            if (Fired(Labeled("Say"), immediateMemory))
            {
                ...
                if (phrasesToSay.Count != 0)
                {
                    ...
                    ModuleSpeakPhonemes nm = (ModuleSpeakPhonemes)FindModule(typeof(ModuleSpeakPhonemes));
