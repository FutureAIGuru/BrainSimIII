...
namespace BrainSimulator.Modules
{
    public class ModuleUKS2 : ModuleUKS
    {
        ...
        {
            if (mv == null) return;
            for (int i = 0; i < mv.NeuronCount / 2 && i < UKSList.Count; i++)
            {
                if (UKSList[i] == null) continue;
                GetNeuron(i).Label = UKSList[i].Label;
            }
            for (int i = UKSList.Count; i < mv.NeuronCount / 2; i++)
            {
                GetNeuron(i).Label = "";
            }
        }
        ...
        public override void DeleteThing(Thing t)
        {
            ...
            if (i > -1)
            {
                for (int j = i + 1; j < UKSList.Count; j++)
                {
