...
!define APP_NAME "Brain Simulator III"
...
!define HELPURL "https://futureai.guru"
!define UPDATEURL "https://futureai.guru/BrainSimDownload.aspx"
!define ABOUTURL "https://futureai.guru"
...
!include "MUI.nsh"
!define MUI_ICON "bs3icon.ico"
!define MUI_UNICON "bs3icon.ico"
...
Page instfiles /ENABLECANCEL
    ...
    !define MUI_FINISHPAGE_RUN_TEXT "Start Brain Simulator III"
...
Section
	...
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${COMPANY_NAME} ${APP_NAME}" "DisplayIcon" "$\"$INSTDIR\bs3icon.ico$\""
    ...
    createShortCut "$SMPROGRAMS\${COMPANY_NAME}\Uninstall Brain Simulator III.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\unicon.ico"
    ...
    createShortCut "$SMPROGRAMS\${COMPANY_NAME}\Brain Simulator III Firewall Setup.lnk" "$INSTDIR\SetupFirewall.exe" "" "$INSTDIR\bs3icon.ico"
SectionEnd

Section    "Brain Simulator III" BRAINSIM2
	SetShellVarContext current

    #new v3 files
     File /oname=$INSTDIR\bs3icon.ico "bs3icon.ico"
     File /oname=$INSTDIR\Emgu.CV.Platform.NetStandard.dll "..\BrainSimulator\bin\release\net6.0-windows\Emgu.CV.Platform.NetStandard.dll"
     File /oname=$INSTDIR\Emgu.CV.Bitmap.dll "..\BrainSimulator\bin\release\net6.0-windows\Emgu.CV.Bitmap.dll"
     File /oname=$INSTDIR\Naudio.WinForms.dll "..\BrainSimulator\bin\release\net6.0-windows\NAudio.WinForms.dll"
     File /oname=$INSTDIR\Naudio.Core.dll "..\BrainSimulator\bin\release\net6.0-windows\NAudio.Core.dll"
     File /oname=$INSTDIR\Naudio.WinMM.dll "..\BrainSimulator\bin\release\net6.0-windows\NAudio.WinMM.dll"
     File /oname=$INSTDIR\Microsoft.CognitiveServices.Speech.csharp.dll "..\BrainSimulator\bin\release\net6.0-windows\Microsoft.CognitiveServices.Speech.csharp.dll"
     File /oname=$INSTDIR\HelixToolkit.Wpf.dll "..\BrainSimulator\bin\release\net6.0-windows\HelixToolkit.Wpf.dll"
     SetOutPath $INSTDIR\Resources
     File /r /x /oname=$INSTDIR\Resources\KeywordTables ..\BrainSimulator\bin\release\net6.0-windows\Resources\KeywordTables
     SetOutPath $INSTDIR
    #from v2 files
...
#    File /oname=$INSTDIR\Touchless.Vision.pdb "..\BrainSimulator\bin\release\net6.0-windows\Touchless.Vision.pdb"
    ...
    createShortCut "$DESKTOP\Brain Simulator III.lnk" "$INSTDIR\BrainSimulator.exe" "" "$INSTDIR\bs3icon.ico"
    createShortCut "$SMPROGRAMS\${COMPANY_NAME}\Brain Simulator III.lnk" "$INSTDIR\BrainSimulator.exe" "" "$INSTDIR\bs3icon.ico"
...
section "uninstall"
    ...
    #new v3 files
    delete $INSTDIR\bs3icon.ico
    delete $INSTDIR\Emgu.CV.Platform.NetStandard.dll
    delete $INSTDIR\Emgu.CV.Bitmap.dll
    delete $INSTDIR\Naudio.WinForms.dll
    delete $INSTDIR\Naudio.Core.dll
    delete $INSTDIR\NAudio.WinMM.dll
    delete $INSTDIR\Microsoft.CognitiveServices.Speech.csharp.dll
...
#    delete $INSTDIR\Touchless.Vision.pdb
    ...
    delete $INSTDIR\System.speech.dll
    delete $INSTDIR\system.IO.Ports.dll
    delete $INSTDIR\BrainSimulator.deps.json
    RMDIR /r $INSTDIR\runtimes
    RMDIR /r $INSTDIR\Networks

	# Remove Start Menu launchers
	delete "$DESKTOP\Brain Simulator III.lnk"
	delete "$SMPROGRAMS\${COMPANY_NAME}\Brain Simulator III Firewall Setup.lnk"
	delete "$SMPROGRAMS\${COMPANY_NAME}\Brain Simulator III.lnk"
    delete "$SMPROGRAMS\${COMPANY_NAME}\Uninstall Brain Simulator III.lnk"
